...
import com.unco.reanimators.util.*;
...
import static com.unco.reanimators.util.EntityUtil.calculateLookAt;
@Module.Info(name = "GloomCrystal", category = Module.Category.COMBAT)
public class GloomCrystal extends Module {
	...
	private Setting<Boolean> thing = register(Settings.b("MutltiPlace", false));
	...
	private Setting<Integer> MinDmg = register(Settings.integerBuilder("Min Dmg").withMinimum(0).withMaximum(16).withValue(13));
    ...
    public void onWorldRender(RenderEvent event) {
        if (render != null) {
            ...
            ReanimatorsTessellator.drawBox(render, Red.getValue().intValue(), Green.getValue().intValue(), Blue.getValue().intValue(), Alpha.getValue().intValue(), GeometryMasks.Quad.ALL);
    ...
    protected int onEnable() {
        if(alert.getValue()) {
            Command.sendChatMessage(" \u00A7eGloomCrystal -> \u00A7aEnabled!");
        }
        return 0;
    }
    ...
    public void onDisable() {
        if(alert.getValue()) {
            Command.sendChatMessage(" \u00A7eGloomCrystal <- \u00A74Disabled!");
        }
        render = null;
        renderEnt = null;
        resetRotation();
